...
def initialize_empty_vectorstores():
  ...
  for provider in MODEL_OPTIONS.keys():
    if provider == "gemini" and not GOOGLE_API_KEY:
      logger.warning("Skipping Gemini vectorstore initialization because GOOGLE_API_KEY is not set.")
      continue
...
def find_similar_chunks(model_provider: str, query: str, top_k: int = 5):
  logger.debug(f"Searching for similar chunks for provider: {model_provider}")
  vectorstore = load_vectorstore(model_provider)
  return hybrid_retrieve(vectorstore, query, top_k=top_k)
