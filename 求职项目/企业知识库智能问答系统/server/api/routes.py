...
from core.llm_chain_factory import answer_question
from core.retrieval import serialize_document
...
async def get_vectorstore_search(request: SearchQueryRequest):
  try:
    ...
    results = find_similar_chunks(model_provider, request.query, top_k=request.top_k)
...
async def chat(request: ChatRequest):
  try:
    ...
    response = answer_question(model_provider, model_name, vectorstore, message, top_k=request.top_k)
