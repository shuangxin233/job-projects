...
题目只要求真实 LLM API，没有指定服务商，也没有要求必须付费。本版默认接入 **智谱免费模型 GLM-4-Flash-250414**；已完成 8 项真实 API 联调；换到其他电脑需配置自己的智谱 Key。申请方式见 [FREE_API_SETUP.md](FREE_API_SETUP.md)。
...
仅在第一次配置时复制，避免覆盖自己已有的配置。登录[智谱开放平台](https://bigmodel.cn)，在控制台创建 API Key，使用免费 glm-4-flash-250414，无需购买 Coding Plan。在 `.env` 填写你自己的智谱 Key：
...
默认智谱模型 glm-4-flash-250414 使用 `max_tokens=1024` 限制输出，工具仍由本地 Python 执行。若手动选择 glm-4.7-flash 或 glm-4.6v-flash，客户端额外设置 `thinking.type=disabled`。

为减少免费接口连续调用触发限流，真实客户端默认在同一进程的相邻请求之间间隔至少 10 秒。可在 `.env` 设置 `LLM_MIN_INTERVAL_SECONDS=10`（0～30 秒）。一次任务包含多次工具决策时可能需要几十秒。不同终端各自限速，不是全账号共享限速器。
...
| 2 | 最近最多 4 个完整历史轮次 | 原始 `user/assistant/tool` 消息 | 支持“刚才那个”“再改短一点”等追问 |
| 3 | 旧对话摘要、完整待办、最近各工具结果 | 当前问题前的独立 `user` 数据消息 | 提高记忆可见性，不把摘要提升成系统指令 |
| 4 | 当前输入、本轮调用和结果 | 原始消息 | 让模型知道本次已完成什么以及下一步如何行动 |
| 5 | 本轮已成功工具及参数清单 | 临时 `user` 消息 | 提醒避免重复、继续遗漏操作；包含在预算中，不额外存入历史 |
...
| 智谱 429 | 只提取安全的数字业务码区分问题；1113 为账户欠费，不自动重试；其他限流遵循等待规则 |
...
| 连续请求刚刚成功执行的相同工具和参数 | 拦截重复执行，将错误回填给模型，提示根据已有结果回答或执行不同操作 |
